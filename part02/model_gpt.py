...
# ______________________Blocks_____________________________
class CausaleSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) : # (B,T,C)
        ...
        # Pytorch SDPA usese flash when available
...
class Block(nn.Module):
    def __init__(self, n_embd, n_head: int, dropout: float):
        super().__init__()
        self.ln1 = nn.LayerNorm(n_embd)
        self.attn = CausaleSelfAttention(n_embd, n_head, dropout)
    ...
    def forward(self, x):
        ...
        return x
# ____________TINY GPT__________________________
