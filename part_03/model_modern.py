...
# Get the absolute path to the folder that contain  part02 and part_03
import os , sys
...
class GPTModern(nn.Module):
    ...
    def generate(self,
        ...
        for _ in range(max_new_tokens):
            # feed full prompt once: tehn only the last token
            
            idx_cond = idx[:, -self.block_size:] if kvs[0] is None else idx[:, -1:]
            # absolute start position form cache length (0 on first step)
