...
if __name__ == "__main__": 
    ...
    k= k.transpose(1, 2)
    v = v.transpose(1, 2)
    log(f"tranpose heads:    q={tuple(q.shape)} k={tuple(k.shape)} v={tuple(v.shape)} = (B, heads, T, d_head)")
    ...
    log("\nLegend:")
    log("B=batch, T=sequence length, d_model=embedding size, heads=n_heads, d_head=d_model/heads")
    log( "qkv(x) is a single Linear proddcing [Q|K|V] ; we reshape then split into q, k, v")
